...
def Generate_Orders(file:str, products: list, customers: list = None,  set_size: int = 100, show: bool = False):
    ...
    for i, customer in enumerate(customers):
        ...
        # random order id.
        order = Order(customer, int(ord_nr + i))
    ...
    with io.open(path, "w+", encoding="utf-8") as file_orders:
        for order in order_list:
            file_orders.write(f"{order.order_id};{np.around(order.total, decimals=2)};{order.person.customer_id},")
...
if __name__ == "__main__":
    ...
    Generate_Orders(test_orders_save, test_products, set_size=10000, show=True)
